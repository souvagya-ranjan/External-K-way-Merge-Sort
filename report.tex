        ...
        \begin{itemize}
            ...
            \item \textbf{merge\_runs(const vector $<$string$>$ \&sorted\_files, const int file\_count,
                ...
                \begin{enumerate}[label = \star]
                    ...
                    \item \textbf{Key Points}: 
                        \begin{enumerate}
                            % \item We store the pointer of the Reader class for each sorted file we want to merge in a vector and reserve memory space for it initially.
                            % \item We keep a array completed which records if a input file for merge is completed or not. The coumpleted\_count keeps a count of the completed files.
            ...
            \item \textbf{external\_merge\_stop\_withstop(const char *input, const char *output,
                ...
                \begin{enumerate}[label=\star]
                    ...
                    % The function uses a k-way merge algorithm, where k is a user-defined parameter. The number of merges can also be specified as a parameter or set to 0 for unlimited merges. 
                    The function sorts the input file using either a trie-based or a run-based sorting algorithm.
                    % The sorted runs are stored in temporary files on disk, and the merging of runs is performed using a merge sort algorithm. The function also measures the time taken to sort and merge the runs and prints it to the console. 
                    
                    % If the input file is empty, the function creates an empty output file. If the input file contains only one run, the function renames the temporary file to the output file. If the number of runs is larger than one, the function merges the runs into larger runs until there is only one run left. Finally, the function returns the number of merges performed. 
                    \item \textbf{Key Points}: 
                        \begin{enumerate}
%                             \item To calculate time, we have chrono library.
%                             \begin{lstlisting}[style = cpp]
% auto time = std::chrono::high_resolution_clock::now();
%                             \end{lstlisting}
                            % \item It first reads the files and sorts the individual chunks using \emph{sort\_in\_runs()} function.
                            % \item It then merges the runs into 1 files using the merge\_runs() function. In each merge step, it merges at most k runs from the remaining runs to be merged in that stage.
                            % \item Once all the runs in a stage are merged, the stage is incremented.
                            % \item We keep a count of runs in each stage of the k-way merge. The stage number and run number of each stage are used to identify the output file name for each stage.
